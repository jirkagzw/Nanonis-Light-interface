# Nanonis-TCP-client
This TCP client package is written in Python to communicate with the Nanonis software. The communication is established over the TCP programming interface which is available for **V5e or V5** software version. This package is based on the https://github.com/LNS-EPFL/Nanonis-TCP-client developed mostly by Shixuan Shan. We extended this with tcp_andor_ctrl.py for tcp communication with linux computer connected to Andor spectrograph and camera. Moreover, photon_meas.py is the highest library now, where functions with simple syntax (originally programmed in our Labview case structure) are available. This library also contains functions that perform the simultaneous operation of Nanonis and spectrograph, e.g., photon_map, spectrum2. Several bugs are fixed compare to the original package. Docstrings in the functions were added and __init__.py was modified (to import manually every class from a .py file) such that now the help is properly showing up also in Spyder and other GUIs. 
