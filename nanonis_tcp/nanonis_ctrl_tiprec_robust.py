...
####################################### Tip Move Recorder #############################################  
    def TipRecDataGet(self, prt=if_print, debug=False, return_meta=False):
        """
        Robust version of TipRec.DataGet.

        The Nanonis response format is:
            int                 number of channels
            1d int              channel indexes, length = number of channels
            int                 data rows
            int                 data columns
            2d float32          data, shape = (data rows, data columns)
            error block         uint32 status + int32 text size + text

        Requires a tcp_ctrl.res_recv() that reads the full response body exactly
        from the size declared in the 40-byte Nanonis response header.
        """
        header = self.tcp.header_construct('TipRec.DataGet', 0)

        # If tcp_ctrl has the robust lock, keep send+receive atomic for this command.
        lock = getattr(self.tcp, '_io_lock', None)
        if lock is None:
            class _NoLock:
        ...
        with lock:
            ...
            res_header, res_arg, res_err = self.tcp.res_recv(
                'int', '1dint', 'int', 'int', '2dfloat32', debug=debug
            )

        self.tcp.print_err(res_err)

        num_channels = int(res_arg[0])
        ch_idx = res_arg[1]
        rows = int(res_arg[2])
        cols = int(res_arg[3])
        data = res_arg[4]

        # Hard consistency checks; fail early instead of silently returning shifted data.
        if len(ch_idx) != num_channels:
            raise ValueError(
                f"TipRec.DataGet parse mismatch: num_channels={num_channels}, "
                f"len(ch_idx)={len(ch_idx)}"
            )
        if data.shape != (rows, cols):
            raise ValueError(
                f"TipRec.DataGet parse mismatch: header shape=({rows}, {cols}), "
                f"data.shape={data.shape}"
            )
        if cols != num_channels:
            # In most TipRec outputs columns correspond to recorded channels.
            # Keep this as a warning, not an exception, because some versions may
            # append/transpose differently.
            print(f"Warning: TipRec data columns ({cols}) != number of channels ({num_channels}).")

        if prt:
            print("TipRec number of channels:", num_channels)
            print("TipRec channel indexes:", ch_idx)
            print("TipRec data rows:", rows)
            print("TipRec data columns:", cols)
            print("TipRec data shape:", data.shape)
            if not res_err.empty:
                print("TipRec error status:", res_err.loc[0, 'error status'])
                print("TipRec error body size:", res_err.loc[0, 'error body size'])
                if res_err.loc[0, 'error body size']:
                    print("TipRec error description:", res_err.loc[0, 'error description'])
